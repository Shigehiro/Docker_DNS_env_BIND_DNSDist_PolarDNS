...
for i in range(0,number_of_zone):
    domain_number = str(f"{i:06}")
    with open(f'../bind_config/sub.example.com/records/sub{domain_number}.delay.com.template.db', 'w') as f:
        conf_string = """
$TTL 60
@ IN SOA ns01 ns01 (
2010062303
...
for i in range(0,number_of_zone):
    domain_number = str(f"{i:06}")
    with open(f'../bind_config/sub.example.com/records/sub{domain_number}.delay.com.template.db', 'a') as f:
        conf_string = f"""
{zone_prefix}{domain_number} IN NS ns01.{zone_prefix}{domain_number}
{zone_prefix}{domain_number} IN NS ns02.{zone_prefix}{domain_number}
...
# Generate named.conf for sub.delay.com
for i in range(0,number_of_zone):
    domain_number = str(f"{i:06}")
    with open('../bind_config/sub.example.com/config/named.conf', 'a') as f:
        conf_string = f"""
zone "{zone_prefix}{domain_number}.{zone_suffix_delay}" in {{
  type master;
  file "/var/lib/bind/sub{domain_number}.delay.com.template.db";
}};
"""
        f.write(conf_string)
